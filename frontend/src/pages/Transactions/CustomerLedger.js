...
const CustomerLedger = () => {
  ...
  // Date states default to current year-month
  ...
  // Load customer master & company config
  const loadInitialData = async () => {
    try {
      ...
      setError('Failed to initialize customer ledger requirements.');
    }
  };
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      let url = `/reports/customer-ledger/${selectedCustomer.id}`;
      const params = [];
      ...
      setError('Failed to load customer ledger report.');
      setLedgerData(null);
    } finally {
      setLoading(false);
    }
  ...
  // Trigger search on customer/date change
  useEffect(() => {
    if (selectedCustomer) {
      handleSearch();
    } else {
  ...
  }, [selectedCustomer, startMonth, endMonth, activeBranchId]);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Date', 'Transaction Type', 'Reference No', 'Debit (Billed)', 'Credit (Paid)', 'Running Balance'];
    ...
    link.setAttribute('download', `CustomerLedger_${selectedCustomer.name.replace(/\s+/g, '_')}_${startMonth || 'all'}_to_${endMonth || 'all'}.csv`);
      ...
      <Card sx={{ mb: 4, borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
        <CardContent sx={{ p: 3 }}>
          <Typography variant="h6" sx={{ fontWeight: 700, mb: 2, color: 'primary.main' }}>
            Customer Ledger Statement Filters
          </Typography>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={5}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Select Customer" variant="outlined" size="small" fullWidth />
                )}
              />
            </Grid>
            <Grid item xs={6} md={3}>
            ...
            <Box sx={{ mt: 3, p: 2, backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={3}>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>{selectedCustomer.name}</Typography>
                </Grid>
                <Grid item xs={6} sm={2}>
                  <Typography variant="caption" color="textSecondary" display="block">GSTIN</Typography>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>{selectedCustomer.gstin || 'N/A'}</Typography>
                </Grid>
                <Grid item xs={6} sm={2}>
                ...
                <Grid item xs={12} sm={5}>
                  <Typography variant="caption" color="textSecondary" display="block">Billing Address</Typography>
                  <Typography variant="body2">{selectedCustomer.billing_address || 'N/A'}</Typography>
                </Grid>
              </Grid>
            </Box>
          )}
            ...
            <Grid item xs={12} sm={4}>
              <Card sx={{
                ...
                <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 3 }}>
                  <Box>
                    <Typography variant="body2" sx={{ opacity: 0.85, fontWeight: 500 }}>Total Billed</Typography>
                    <Typography variant="h4" sx={{ fontWeight: 800, mt: 0.5 }}>₹{ledgerData.total_billed.toFixed(2)}</Typography>
                  </Box>
            ...
            <Grid item xs={6} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'center' }}>
              <Paper variant="outlined" sx={{ p: 2, width: '80%', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                ...
                <Grid container spacing={1} sx={{ mt: 1 }}>
                  <Grid item xs={7}>
                    <Typography variant="body2" color="textSecondary">Total Billed:</Typography>
                  </Grid>
                  <Grid item xs={5} sx={{ textAlign: 'right' }}>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>₹{ledgerData?.total_billed.toFixed(2)}</Typography>
                  </Grid>
                  <Grid item xs={7}>
